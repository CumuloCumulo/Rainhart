...
function loadConfig() {
  chrome.storage.local.get(['allowedDomains', 'extractOptions'], (result) => {
    currentConfig.allowedDomains = result.allowedDomains || [...DEFAULT_CONFIG.allowedDomains];
...
function saveConfig() {
  // 过滤空域名
  currentConfig.allowedDomains = currentConfig.allowedDomains.filter(d => d.trim() !== '');
  ...
  chrome.storage.local.set(currentConfig, () => {
    if (chrome.runtime.lastError) {
      ...
      console.log('[小红书提取器 Options] 注意：域名更改需要重新加载插件才能生效');
    }
  });
}
